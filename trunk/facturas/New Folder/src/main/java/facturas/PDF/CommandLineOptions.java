...
/* $Id: CommandLineOptions.java 542237 2007-05-28 14:31:24Z jeremias $ */
...
public class CommandLineOptions {
	...
	private PDFGeneador _generador = null;
	 ...
	 */
	public CommandLineOptions(PDFGeneador geneador) {
		_generador = geneador;
	...
	public void parse(String[] args) throws FOPException, IOException {
		...
		if (MimeConstants.MIME_FOP_AWT_PREVIEW.equals(outputmode)) {
			...
			Renderer targetRenderer = foUserAgent.getRendererFactory()
					.createRenderer(foUserAgent, mimicRenderer);
	...
	private int parseConfigurationOption(String[] args, int i)
			throws FOPException {
	...
	private int parsePrintOutputOption(String[] args, int i)
			throws FOPException {
	...
	private int parsePostscriptOutputOption(String[] args, int i)
			throws FOPException {
	...
	private int parseCustomOutputOption(String[] args, int i)
			throws FOPException {
		String mime = null;
		if ((i + 1 < args.length) || (args[i + 1].charAt(0) != '-')) {
			mime = args[i + 1];
			if ("list".equals(mime)) {
				String[] mimes = factory.getRendererFactory()
		...
		if ((i + 2 >= args.length) || (args[i + 1].charAt(0) == '-')
				|| (args[i + 2].charAt(0) == '-')) {
			throw new FOPException(
	...
	private int parseAreaTreeInputOption(String[] args, int i)
			throws FOPException {
	...
	private PDFEncryptionParams getPDFEncryptionParams() throws FOPException {
		PDFEncryptionParams params = (PDFEncryptionParams) renderingOptions
				.get(PDFRenderer.ENCRYPTION_PARAMS);
		if (params == null) {
	...
	private void checkSettings() throws FOPException, FileNotFoundException {
		...
		if ((outputmode.equals(MimeConstants.MIME_FOP_AWT_PREVIEW) || outputmode
				.equals(MimeConstants.MIME_FOP_PRINT))
				&& outfile != null) {
		...
		if (inputmode == XSLT_INPUT) {
			...
			if (xmlfile != null || xsltfile != null) {
				 ...
				 * if (!fofile.exists()) { throw new
				 * FileNotFoundException("Error: fo file " +
			...
			if (xmlfile != null || xsltfile != null) {
				log
						.warn("area tree input mode, but xmlfile or xslt file are set:");
				log.error("xml file: " + xmlfile.toString());
				log.error("xslt file: " + xsltfile.toString());
			}
			if (!areatreefile.exists()) {
				throw new FileNotFoundException("Error: area tree file "
						+ areatreefile.getAbsolutePath() + " not found ");
			}
		}
	} // end checkSettings
	...
	private InputHandler createInputHandler() throws IllegalArgumentException {
		switch (inputmode) {
		case FO_INPUT:
			return new InputHandler(fofile, _generador);
		case AREATREE_INPUT:
			return new AreaTreeInputHandler(areatreefile,_generador);
		case XSLT_INPUT:
			return new InputHandler(xmlfile, xsltfile, xsltParams, _generador);
		default:
	...
	private void printUsagePrintOutput() {
		System.err
				.println("USAGE: -print [-Dstart=i] [-Dend=i] [-Dcopies=i] [-Deven=true|false] "
	...
	private void dumpConfiguration() {
		...
		if (userConfigFile != null) {
			log.info("user configuration file: " + userConfigFile.toString());
		} else {
